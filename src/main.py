...
class BrowserRecycleRequired(Exception):
    """
    Raised to request a clean browser relaunch - not an error.

    Chromium's RSS grows steadily with uptime; left alone it walks into the
    unit's MemoryMax and gets OOM-killed at an arbitrary moment, usually
    mid-cycle. Recycling deliberately, at a chosen threshold, turns an
    unplanned kill into a controlled restart that reloads cookies and carries
    on. The outer loop treats this as a normal event and does NOT count it
    against restart_attempts.
    """


def _cgroup_memory_mb() -> float:
    """
    Memory as systemd accounts it: the cgroup's memory.current, in MB.

    This must match what MemoryMax is enforced against, or the recycle
    threshold is meaningless. Summing RSS across the Chromium process tree
    does NOT match: RSS counts shared pages once per process, and Chromium's
    zygote/renderer tree shares heavily. Measured on this host, the RSS sum
    read 1568MB while the cgroup read 344MB - a 4.5x overcount that fired a
    recycle every ten minutes.

    Returns 0.0 if the cgroup file is unavailable, so callers fall back to
    the uptime trigger rather than recycling on a bogus reading.
    """
    try:
        # cgroup v2 (Ubuntu 22.04): "0::/system.slice/<unit>.service"
        ...
        current = Path("/sys/fs/cgroup") / rel.lstrip("/") / "memory.current"
        if current.exists():
            return int(current.read_text().strip()) / (1024 * 1024)
    except Exception:
        pass
    return 0.0
...
async def run_daemon():
    ...
    while restart_attempts < max_restarts:
        try:
            ...
            # A working session clears the failure budget. Without this,
            # unrelated failures spread over days accumulate until the daemon
            # gives up permanently despite running fine in between.
            restart_attempts = 0
            ...
            break
        except BrowserRecycleRequired as e:
            # Planned relaunch, not a failure - loop round without spending
            # any of the restart budget. The finally block stops the engine.
            log.info("main.recycling_browser", reason=str(e))
...
async def _health_loop(
    ...
    recycle_hours = get("browser.recycle_hours", 6)
    recycle_mb = get("browser.recycle_memory_mb", 750)
    started = time.monotonic()

    while True:
        await asyncio.sleep(interval)

        # ── Planned browser recycle ───────────────────────────────────
        # Checked before the health probe: if Chromium is close to the
        # cgroup limit we want a controlled relaunch, not an OOM kill
        # partway through a cycle.
        age_hours = (time.monotonic() - started) / 3600.0
        mem_mb = _cgroup_memory_mb()
        log.info("health_loop.resources",
                 uptime_hours=round(age_hours, 2), cgroup_mb=round(mem_mb))

        # mem_mb == 0 means the cgroup file was unreadable; don't recycle on
        # an unknown value, just fall through to the uptime trigger.
        if mem_mb and mem_mb >= recycle_mb:
            await save_cookies(context)
            raise BrowserRecycleRequired(
                f"memory {mem_mb:.0f}MB >= {recycle_mb}MB"
            )
        if age_hours >= recycle_hours:
            await save_cookies(context)
            raise BrowserRecycleRequired(
                f"uptime {age_hours:.1f}h >= {recycle_hours}h"
            )
